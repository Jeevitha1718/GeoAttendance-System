...
// Global variable to store captured location during registration
let capturedLocation = null;

const MODEL_URL = 'D:\Kirthihaa\CSE-1\Placement\mini project\geolocation based attendance tracker\models';
...
// Toggle geofence section based on role selection
...
function getCurrentLocationForRegistration() {
  const locationStatus = document.getElementById("location-status-reg");
  ...
  if (!navigator.geolocation) {
    locationStatus.innerHTML = '<span class="location-error">Geolocation is not supported by your browser</span>';
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      // Update UI
      document.getElementById("captured-lat").textContent = `Latitude: ${latitude.toFixed(6)}`;
      document.getElementById("captured-lon").textContent = `Longitude: ${longitude.toFixed(6)}`;
      
      locationCoordinates.style.display = 'block';
      locationStatus.innerHTML = '<span class="location-success">Location captured successfully!</span>';
      getLocationBtn.disabled = false;
    ...
    (error) => {
      ...
      locationStatus.innerHTML = `<span class="location-error">${errorMessage}</span>`;
      getLocationBtn.disabled = false;
      capturedLocation = null;
    },
    options
...
async function registerUser(event) {
  ...
  // For users, validate location capture
  if (role === "user") {
    if (!capturedLocation) {
      errorMessage.textContent = "Please capture your location by clicking 'Get Current Location' button.";
      errorMessage.style.display = "block";
      return;
    }
  }
  ...
  // For users, require face registration
  if (role === "user") {
    try {
      const faceRegistered = await showFaceRegistrationModal(email);
      if (!faceRegistered) {
        errorMessage.textContent = "Face registration is required for users.";
        errorMessage.style.display = "block";
        return;
      }
    } catch (error) {
  ...
  // Save user data with geofence information for users
  ...
  // Add geofence data for users
  if (role === "user") {
    const radius = document.getElementById("geofence-radius").value || 100;
    userData.geofence = {
  ...
  // Initialize empty attendance data for new users
  if (role === "user") {
...
function showRegister() {
  ...
  capturedLocation = null;
  document.getElementById("geofence-section").style.display = 'none';
  document.getElementById("location-coordinates").style.display = 'none';
  document.getElementById("location-status-reg").innerHTML = 'Location: Click the button below to get your current location';
}
...
function isWithinAnyGeofence(userLat, userLng, userEmail) {
  ...
  if (!user || !user.geofence) {
    console.error("No geofence data found for user:", userEmail);
    return false;
  }
  
  const userGeofence = user.geofence;
  const distance = getDistanceFromLatLonInMeters(
    userLat,
    userLng,
    userGeofence.latitude,
    userGeofence.longitude
  );
  
  console.log(`Distance from geofence: ${distance.toFixed(2)} meters, Radius: ${userGeofence.radius} meters`);
  return distance <= userGeofence.radius;
}
...
function loadAllAttendanceData() {
  ...
  // Get all users data to map emails to usernames
  const users = JSON.parse(localStorage.getItem("users")) || [];
  
  // Create a mapping of email to username
  const emailToUsernameMap = {};
  users.forEach(user => {
    emailToUsernameMap[user.email] = user.name || user.email; // Fallback to email if name doesn't exist
  });
  ...
  allKeys.forEach(key => {
    if (key.startsWith("attendanceData_")) {
      ...
      // Get username from mapping, fallback to email if not found
      ...
      attendanceRecords.forEach(record => {
        allAttendanceData.push({
          ...
          email, // Keep email for reference if needed
          date: record.date,
...
function exportAllToPDF() {
  ...
  // Set the title
  doc.setFontSize(18);
  doc.text("All User Attendance Records", 10, 10);
